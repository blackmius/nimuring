...
template withUdpSockets(body: untyped) =
  let server {.inject.} = newSocket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
  let client {.inject.} = newSocket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
  try:
    server.bindAddr()
    let (ip {.inject.}, port {.inject.}) = server.getLocalAddr()
    body
  finally:
    server.close()
    client.close()

suite "sendmsg/recvmsg operation":
  test "sendmsg and recvmsg basic":
    withUdpSockets:
      var q = newQueue(4)
  ...
  test "sendmsg/recvmsg userData and result":
    withUdpSockets:
      var q = newQueue(4)
      var bufferSend = ['a', 'b', 'c', 'd', 'e', 'f']
      var iovecSend = IOvec(iov_base: bufferSend.addr, iov_len: bufferSend.len.uint)
      ...
      q.sendmsg(123, client.getFd, msgSend.addr, 0).linkNext()
      ...
      q.recvmsg(456, server.getFd, msgRecv.addr, 0)
      q.submit(2)

      let cqes = q.copyCqes(2)
      check cqes.len == 2
      check (cqes[0].userData == 123 or cqes[1].userData == 123)
      check (cqes[0].userData == 456 or cqes[1].userData == 456)
      check bufferRecv == bufferSend